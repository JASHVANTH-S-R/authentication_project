...
def detect_faces(stream_path):
    ...
    if frame is None:
        print(f"Error: Unable to load image from path: {stream_path}")
        return False

    encodings, faces = load_encodings()
    ...
    # Iterate over detected faces and compare with known encodings
    ...
    # Check if any face was detected and recognized
    if name is not None:
        if name in data['face']['ids']:
